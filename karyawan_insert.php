...
if (isset($_POST["submit_i"])) {
    ...
    $query = "INSERT INTO tbl_pengguna (pengguna_nama, username, password, pengguna_pilihan) VALUES ('$pengguna_nama', '$username', '$password', 'Karyawan')";
    $submit_i = mysqli_query($config, $query);

    if ($submit_i) {
        echo '<script>alert("Karyawan berhasil ditambahkan.");window.location="karyawan.php"</script>';
    } else {
        echo '<script>alert("Terjadi kesalahan saat menambahkan karyawan.");</script>';
    }
}
?>
